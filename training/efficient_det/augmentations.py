...
def mixup_batch(images, labels, alpha=0.2):
    ...
    image_lam = tf.reshape(lam, [1, 1, 1, 1])
    label_lam = tf.reshape(lam, [1, 1])
    mixed_images = image_lam * images + (1.0 - image_lam) * shuffled_images
    mixed_labels = label_lam * labels + (1.0 - label_lam) * shuffled_labels
    return mixed_images, mixed_labels


def cutmix_batch(images, labels, min_lambda=0.35, max_lambda=0.8):
    batch_size = tf.shape(images)[0]
    indices = tf.random.shuffle(tf.range(batch_size))
    shuffled_images = tf.gather(images, indices)
    shuffled_labels = tf.gather(labels, indices)
    ...
    lam = tf.random.uniform([], min_lambda, max_lambda)
    cut_ratio = tf.sqrt(1.0 - lam)
    cut_height = tf.maximum(1, tf.cast(tf.cast(height, tf.float32) * cut_ratio, tf.int32))
    cut_width = tf.maximum(1, tf.cast(tf.cast(width, tf.float32) * cut_ratio, tf.int32))

    center_y = tf.random.uniform([], 0, height, dtype=tf.int32)
    center_x = tf.random.uniform([], 0, width, dtype=tf.int32)
    y1 = tf.clip_by_value(center_y - cut_height // 2, 0, height)
    y2 = tf.clip_by_value(center_y + cut_height // 2, 0, height)
    x1 = tf.clip_by_value(center_x - cut_width // 2, 0, width)
    x2 = tf.clip_by_value(center_x + cut_width // 2, 0, width)

    top = tf.ones([y1, width, 1], dtype=images.dtype)
    middle_left = tf.ones([y2 - y1, x1, 1], dtype=images.dtype)
    middle_center = tf.zeros([y2 - y1, x2 - x1, 1], dtype=images.dtype)
    middle_right = tf.ones([y2 - y1, width - x2, 1], dtype=images.dtype)
    middle = tf.concat([middle_left, middle_center, middle_right], axis=1)
    bottom = tf.ones([height - y2, width, 1], dtype=images.dtype)
    ...
    lam_adjusted = 1.0 - cut_area / tf.cast(height * width, tf.float32)
    lam_labels = tf.reshape(lam_adjusted, [1, 1])
    mixed_labels = lam_labels * labels + (1.0 - lam_labels) * shuffled_labels
    return mixed_images, mixed_labels


def apply_mixup_or_cutmix(images, labels, probability=0.7):
    def mixup():
    ...
    return tf.cond(tf.random.uniform([]) < probability, mixup, cutmix)